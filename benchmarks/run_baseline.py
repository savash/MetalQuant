...
def run_one(model, tokenizer, prompt: str, max_new_tokens: int) -> dict:
    cache_list = [KVCache() for _ in range(len(model.layers))]
...
def main() -> None:
    ...
    runs = [run_one(model, tokenizer, prompt, config.max_new_tokens) for prompt in PROMPTS]
    summary = {
        "avg_prefill_tok_per_s": sum(run["prefill_tok_per_s"] for run in runs) / len(runs),
        "avg_decode_tok_per_s": sum(run["decode_tok_per_s"] for run in runs) / len(runs),
        "avg_decode_latency_ms": sum(run["avg_decode_latency_ms"] for run in runs) / len(runs),
        "avg_cache_bytes_after_decode": sum(run["cache_bytes_after_decode"] for run in runs) / len(runs),
    }
